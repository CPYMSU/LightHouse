...
class FakeMemory:
    ...
    def context(self, **kwargs):
        return {
            "conversation": {
                "id": "conversation-1",
                "active_subject_kind": "file",
                "active_subject_value": str(self.active_file),
            },
            "active_task": {
                "subject_kind": "file",
                "subject": str(self.active_file),
            },
            "relevant_files": [
                {"canonical_path": str(self.active_file), "name": self.active_file.name}
            ],
            "recent_locators": [
                {
                    "kind": "file",
                    "canonical_value": str(self.active_file),
                    "display_value": str(self.active_file),
                }
            ],
        }
...
def test_random_existing_cwd_is_rejected_when_not_in_memory(tmp_path):
    ...
    with pytest.raises(ValueError, match="not observed"):
        resolver.ground(
            run=run,
            capability=capability,
            arguments={"cwd": str(random), "command": "pwd"},
        )


def test_new_directory_is_relative_to_bound_workspace(tmp_path):
    resolver, run, _active, random = setup_resolver(tmp_path)
    grounded = resolver.ground(
        run=run,
        capability=DIRECTORY_CREATE_CAPABILITY,
        arguments={"cwd": str(random), "path": "web", "parents": True},
    )
