...
def test_long_single_line_final_message_is_not_truncated():
    stream = StringIO()
    ui = SwissTerminal(Console(file=stream, width=52, color_system=None, force_terminal=False, highlight=False))
...
def test_long_single_line_input_card_is_not_truncated():
    stream = StringIO()
    ui = SwissTerminal(Console(file=stream, width=48, color_system=None, force_terminal=False, highlight=False))
