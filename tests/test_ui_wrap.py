...
from lighthouse.terminal_entry import _redraw
...
def test_terminal_folio_uses_installed_package_version():
    stream = StringIO()
    ui = SwissTerminal(
        Console(
            file=stream,
    ...
    output = stream.getvalue()
    assert f"FOLIO {__version__}" in output
    assert "FOLIO 0.7" not in output
