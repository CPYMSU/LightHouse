...
def test_confirm_deferred_returns_running_and_receipt_is_recoverable():
    ...
    target = repository.create_target(
        ...
        config={"transport": "local", "default_cwd": "/tmp", "allowed_roots": ["/tmp"]},
    )
    ...
    capability = Capability(
        tool_name="system.test.blocking.v1",
        command="blocking",
        description="blocking test operation",
    ...
    repeated = kernel.confirm_deferred(operation_id, actor="operator")
    assert repeated["operation"]["status"] == "running"
    ...
    assert final["receipt"]["ok"] is True
