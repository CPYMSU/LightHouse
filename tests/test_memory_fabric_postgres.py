...
def test_messages_tasks_locators_and_files_survive_without_keyword_inheritance(tmp_path):
    suffix = uuid4().hex[:10]
    repository = PostgresRepository(DSN)
    repository.migrate(migration_sql())
    target = repository.create_target(
        name=f"memory-system-{suffix}",
    ...
    memory.record_message(
        ...
        role="user",
        content="把桌面的 index.html 做得酷炫一些",
        run_id=run1,
    )
    memory.start_task(
        run_id=run1,
        conversation_id=conversation["id"],
        goal="把桌面的 index.html 做得酷炫一些",
    )
    memory.project_operation(
        run1,
        {
            "operation": {
                "capability": "system.file.write.v1",
                "envelope": {"arguments": {"path": "Desktop/index.html"}},
            },
            "receipt": {
                "ok": True,
                "result": {
                    "path": str(html),
                    "relative_path": "Desktop/index.html",
    ...
    same = memory.ensure_conversation(
        workspace_id=workspace.id,
        actor="adsin",
        conversation_id=conversation["id"],
    )
    ...
    assert any(
        item["canonical_path"] == str(html.resolve())
        for item in context["relevant_files"]
    )
    assert [
        turn["user"]["content"] for turn in context["recent_turns"]
    ][-2:] == [
        "把桌面的 index.html 做得酷炫一些",
        "感覺內容還是不夠",
    ]
...
def test_workspace_scan_builds_searchable_file_and_directory_index(tmp_path):
    ...
    assert any(
        item["kind"] == "directory"
        and item["canonical_value"] == str(nested.resolve())
        for item in context["recent_locators"]
    )
