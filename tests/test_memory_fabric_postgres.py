...
def test_messages_tasks_locators_and_files_survive_across_runs(tmp_path):
    ...
    assert [item["content"] for item in context["recent_messages"]][-2:] == [
        "把桌面的 index.html 做得酷炫一些",
        "再做得豐富一些",
    ]


def test_workspace_scan_builds_searchable_file_index(tmp_path):
    suffix = uuid4().hex[:10]
    repository = PostgresRepository(DSN)
    repository.migrate(migration_sql())
    target = repository.create_target(
    ...
    file = tmp_path / "warehouse-dashboard.html"
