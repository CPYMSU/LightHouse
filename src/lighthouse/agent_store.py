...
class PostgresAgentStore:
    ...
    def create_run(
        ...
        *,
        ...
        with self._connect() as connection:
            ...
            if row is None:
                row = connection.execute(
                    "SELECT * FROM lh_agent_runs WHERE id=%s",
                    (run_id,),
                ).fetchone()
    ...
    def update_run(
        ...
        *,
        ...
        with self._connect() as connection:
            row = connection.execute(
                """UPDATE lh_agent_runs
                   SET status=%s,current_step=%s,pending_operation_id=%s,
                       final_message=%s,updated_at=now()
    ...
    def append_step(self, run_id: str, kind: str, payload: dict[str, Any]) -> dict[str, Any]:
        with self._connect() as connection:
            locked = connection.execute(
                "SELECT id FROM lh_agent_runs WHERE id=%s FOR UPDATE",
                (run_id,),
            ).fetchone()
            if not locked:
                raise KeyError("agent run not found")
            row = connection.execute(
                ...
                (run_id, kind, json.dumps(payload), run_id),
            ).fetchone()
        return {
    ...
    def list_steps(self, run_id: str) -> list[dict[str, Any]]:
        ...
        with self._connect() as connection:
            rows = connection.execute(
                ...
                "FROM lh_agent_steps WHERE run_id=%s ORDER BY sequence",
                (run_id,),
            ).fetchall()
        return [
            {
