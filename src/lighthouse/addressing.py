...
class ExecutionAddressResolver:
    """Resolve model-proposed paths against durable, previously observed locators.
    ...
    def _active_subject(memory: dict[str, Any]) -> Path | None:
        ...
        raw = task.get("subject") or ((memory.get("conversation") or {}).get("active_subject_value") if isinstance(memory.get("conversation"), dict) else None)
        if not raw:
            return None
