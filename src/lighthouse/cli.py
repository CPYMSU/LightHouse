...
def config_path(args) -> Path:
    return Path(args.config or os.environ.get("LIGHTHOUSE_CONFIG") or Path.home() / ".lighthouse" / "config.json").expanduser()
...
class Client:
    def __init__(self, base_url: str, api_key: str, timeout: int = 600):
        ...
        self.client = httpx.Client(base_url=self.base_url, headers={"Authorization": "Bearer " + api_key}, timeout=timeout, follow_redirects=False)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="lh", description="LightHouse OS super terminal")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "migrate":
            ...
            print_json(client.request("POST", "/v1/targets", {"name": args.name, "kind": args.kind, "config": parse_json_object(args.config_json, "--config-json")}))
        elif args.command == "workspaces":
            print_json(client.request("GET", "/v1/workspaces"))
        elif args.command == "workspace-add":
            print_json(client.request("POST", "/v1/workspaces", {"name": args.name, "data_target_id": args.data_target, "system_target_id": args.system_target}))
        elif args.command == "run":
            workspace, mode, actor = context(args, config)
            value = client.request("POST", "/v1/operations", {"capability": args.capability, "arguments": parse_json_object(args.args_json, "--args-json"), "workspace_id": workspace, "actor": actor, "mode": mode, "idempotency_key": args.idempotency_key})
